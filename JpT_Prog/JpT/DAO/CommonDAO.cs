...
namespace JpT.DAO
{
    public class CommonDAO
    {
        ...
        public static void KotobaUpdateDataIsHard(List<KotobaEntity> listKotobaEntity)
        {
        ...
        public static void KanjiUpdateDataIsHard(List<KanjiEntity> listKanjiEntity)
        {
        ...
        public static bool InitData()
        {
            try
            {
                ...
                {
                    bool isLock = !string.IsNullOrEmpty(_excel.cell_GetValueByCell(i, Constant.KOTOBA_COLUMN_LOCK));
                    if (!isLock)
                    {
                ...
                for (int i = 3; i <= countRows; i++)
                {
                    bool isLock = !string.IsNullOrEmpty(_excel.cell_GetValueByCell(i, Constant.KANJI_COLUMN_LOCK));
                    if (!isLock)
                    {
